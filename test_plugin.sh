...
vault write sys/plugins/catalog/helloworld \
    sha_256="$(shasum -a 256 $PWD/bin/vault-plugin-helloworld| cut -d " " -f1)" \
    command="vault-plugin-helloworld"
vault secrets enable --plugin-name='helloworld' --path="hello" plugin
